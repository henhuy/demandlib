...
class ElecSlp:
    ...
    def __init__(self, year, seasons=None, holidays=None):
        ...
        self.year = year
        self.slp_frame = self.all_load_profiles(self.date_time_index, holidays=holidays)

    def all_load_profiles(self, time_df, holidays=None):
        slp_types = ["h0", "g0", "g1", "g2", "g3", "g4", "g5", "g6", "l0", "l1", "l2"]
        new_df = self.create_bdew_load_profiles(time_df, slp_types, holidays=holidays)
    ...
    def create_bdew_load_profiles(self, dt_index, slp_types, holidays=None):
        ...
        index = pd.date_range(pd.datetime(2007, 1, 1, 0), periods=2016, freq="15Min")
        ...
        for p in self.seasons.keys():
            a = pd.datetime(self.year, self.seasons[p][0], self.seasons[p][1], 0, 0)
            b = pd.datetime(self.year, self.seasons[p][2], self.seasons[p][3], 23, 59)
            new_df.update(
                pd.DataFrame.merge(
    ...
    def get_profile(self, ann_el_demand_per_sector):
        ...
        """
        return (
            self.slp_frame.multiply(pd.Series(ann_el_demand_per_sector), axis=1).dropna(
                how="all", axis=1
            )
            * 4
        )
